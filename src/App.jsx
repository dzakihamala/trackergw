...
import {
  Play, Pause, Plus, AlertCircle, Trash2, Repeat, Infinity, CheckCircle2,
  GripVertical, CalendarDays, Settings as SettingsIcon, X, Hash,
  AlertTriangle, Clock, SkipForward, Zap, Coffee
} from 'lucide-react';
...
function App() {
  ...
  // NEW: tickRef to avoid stale closures from worker
  ...
  // Sounds
  const workUrl = `${base}work.mp3`;
  const breakUrl = `${base}break.mp3`;
  const finishUrl = `${base}finish.mp3`;

  const [playWorkDone, workCtl] = useSound(workUrl, {
    volume: 1,
    interrupt: true,
    html5: true,
    onplayerror: () => {
    ...
    onloaderror: (_, err) => console.log("work sound load error:", err),
  });

  const [playBreakDone, breakCtl] = useSound(breakUrl, {
    volume: 1,
    interrupt: true,
    html5: true,
    onplayerror: () => {
    ...
    onloaderror: (_, err) => console.log("break sound load error:", err),
  });

  const [playComplete, completeCtl] = useSound(finishUrl, {
    volume: 1,
    interrupt: true,
    html5: true,
    onplayerror: () => {
      Howler.ctx?.resume().then(() => {
        completeCtl.sound?.play();
      });
    },
    onloaderror: (_, err) => console.log("finish sound load error:", err),
  });
  ...
  // safePlay uses controller sound instance
  const safePlay = (type) => {
    try {
      if (Howler.ctx?.state === "suspended") {
        Howler.ctx.resume();
      }
      if (type === "work") workCtl.sound?.play();
      if (type === "break") breakCtl.sound?.play();
      if (type === "finish") completeCtl.sound?.play();
    } catch (e) {
  ...
  const handleTick = () => {
    ...
    if (!isPomodoroMode) {
      ...
      if (!isOvertime) {
        setTimeLeftMs((prev) => {
          if (prev <= 0) {
            if (!stateRef.current.isOvertime) {
              setIsOvertime(true);
              timerMode === 'work' ? safePlay("work") : safePlay("break");
            }
            return 0;
          }
          return prev - 10;
  ...
  }, [selectedAgenda, isActive, isPomodoroMode]);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (saved) {
        ...
        const validElapsed = elapsed > 86400000 ? 0 : elapsed;
  ...
  const openConfirm = (title, message, action, type = 'normal') => {
    setConfirmation({
      ...
      onConfirm: () => { action(); setConfirmation({ ...confirmation, isOpen: false }); }
    });
  };
  ...
  const toggleSystemMode = () => {
    ...
    if (nextIsPomodoro) {
      ...
      if (currentTask) setTimeLeftMs(currentTask.todayDuration * 1000);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                      {urgentTasks.map((agenda) => (
                        <SortableAgendaCard
                          ...
                          onDelete={() => handleDeleteRequest(agenda)}
                        />
                      ))}
                    </div>
                  </SortableContext>
      ...
      <button
        onClick={() => {
          ...
          console.log("TEST SOUND CLICKED", {
            ...
            breakUrl,
            finishUrl,
            hasWorkSound: !!workCtl.sound,
            hasBreakSound: !!breakCtl.sound,
            hasFinishSound: !!completeCtl.sound,
          });
          safePlay("work");
        }}
        className="fixed bottom-3 right-3 z-[9999] px-3 py-2 text-[10px] font-black uppercase tracking-widest
                   bg-black text-white border-2 border-white shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]
